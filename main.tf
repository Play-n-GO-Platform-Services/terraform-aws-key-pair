...
resource "aws_key_pair" "imported" {
  count      = var.generate_ssh_key == false && var.enabled == true ? 1 : 0
  key_name   = module.label.id
  public_key = file(local.public_key_filename)
}

resource "tls_private_key" "default" {
  count     = var.generate_ssh_key == true && var.enabled == true ? 1 : 0
...
resource "null_resource" "chmod" {
  count      = var.generate_ssh_key == true && var.chmod_command != "" && var.enabled == true ? 1 : 0
